...
static void AdjustDirection(
	...
	int				 iDir )
{
	// Maps N & S to 0, E & W to 2
	int	iViewDir = ( iDir - ( xiDir * QUARTER_ROT )) & 0x007f;
	int	iWeight	= 32 - ( abs( 32 - abs( 32 - (( iViewDir + QUARTER_ROT + EIGHTH_ROT ) & 0x007f ))));

	// Adjust angle due to terrain

	switch ( iTiltIndex )
	{
		case CVehicleSprite::TILT_BACK:    iWeight = 32; 		break;
		case CVehicleSprite::TILT_FLAT:    iWeight = iWeight;	break;
		case CVehicleSprite::TILT_FORWARD: iWeight = iWeight; break;
	}

	drawparms.m_byRotAngle = ( BYTE )( drawparms.m_byRotAngle * iWeight / 32 );

	// Adjust angle due to tweening between sprite views

	switch ( iTiltIndex )
	{
		case CVehicleSprite::TILT_BACK:    iWeight = 0; 		break;
		case CVehicleSprite::TILT_FLAT:    iWeight = iWeight;	break;
		case CVehicleSprite::TILT_FORWARD: iWeight = 32;
	}

	int	iActualDir  = 2 * EIGHTH_ROT * ( iDirectionIndex + 2 * xiDir );
	int	iDeltaAngle = 2 * iDir - iActualDir;

	drawparms.m_byRotAngle += iDeltaAngle * iWeight / 32;
}
...
void CUnit::UpdateStore (BOOL bForce)
{ 
	...
	if ( ( bForce ) || (m_dwLastMatTime + 30 * 1000 < theGame.GettimeGetTime ()) )
		{
		m_iUpdateMat = 0;
...
static int GetDirectionIndex(
	int iDir )
{
	int iDir2 = ((( iDir + FULL_ROT / 16) * 8) / FULL_ROT - xiDir * 2) & 0x07;
...
CHexCoord CBuilding::GetExitDest (CTransportData const * pTd, BOOL bFar)
{
	...
	while ( iTrys-- )
		{
		_sub = CSubHex ( GetExit (pTd->GetWheelType ()) );
		...
		// no unit there
		if ( _sub != GetExit (pTd->GetWheelType ()) )
...
void CBuilding::UpdateConst ( CMsgBldgStat * pMsg )
{
	...
	// can we see it?
	if ( (GetUnitType () != CUnit::building) || (theMap._GetHex ( GetHex() )->GetVisible ()) )
		SetConstPer ();
}
	...
	const CHexCoord & hexcoord )
{
	...
	if ( -1 == m_iVisConstDone || -1 == m_iVisFinalPer || -1 == m_iVisSkltnPer )
		return;
	...
	CPoint	ptCorner = GetDrawCorner( hexcoordLeft );

	drawparms.m_ptOffset    = ptCorner;
	drawparms.m_ptOffset.y -= pspriteview->Height();

	// If foundation not complete, draw partial foundation

	if ( m_iVisFoundPer != -1 )
	{
		CRect	rect = pspriteview->Rect() - pspriteview->Rect().TopLeft() + drawparms.m_ptOffset;

		rect.top += (( 100 - m_iVisFoundPer ) * rect.Height() ) / 100;

		CRect	rectBound = rect & pspriteview->DrawClip( drawparms, &rect );
	...
	const CHexCoord & hexcoord )
{
	...
	// #define TESTING CONSTRUCTION_ANIMS
	#ifdef TESTING_CONSTRUCTION_ANIMS
	ASSERT( ! GetAmbient( CSpriteView::ANIM_FRONT_1 )->IsEnabled() );
	ASSERT( ! GetAmbient( CSpriteView::ANIM_FRONT_2 )->IsEnabled() );
	ASSERT( ! GetAmbient( CSpriteView::ANIM_BACK_1  )->IsEnabled() );
	ASSERT( ! GetAmbient( CSpriteView::ANIM_BACK_2  )->IsEnabled() );
	#endif

	// Get the skeleton (foundation is drawn earlier in DrawFoundation()
	...
	CPoint	ptHotSpotClient )
{
	...
	int		iDirectionIndex = GetDirectionIndex( GetTurret()->GetDir() );
...
int CBuilding::GetBldgMatReq (int iInd) const
{
	...
	CBridgeUnit	const	* pbridge ) const
{
	...
	BYTE	byRotAngle = ( BYTE )( asin( double( iDY ) / double( iDX )) * 128. / 3.14159265 );

	drawparms = CQuadDrawParms(( CVehicle & ) *this, CPoint( 0, 0 ), ptCenter, byRotAngle );
	*/

	drawparms = CQuadDrawParms(( CVehicle & ) *this, CPoint( 0, 0 ), ptCenter, 0 );
	...
	const CHexCoord & )
{
	...
	int	iDirectionIndex = GetDirectionIndex( m_iDir );
	...
	if ( GetTurret() )
	{
		...
		iDirectionIndex = GetDirectionIndex( GetTurret()->GetDir() );
		...
		if ( photspotTurret )
		{
			...
			if ( ptileMuzzleFlash && ptileMuzzleFlash->GetAmbient( CSpriteView::ANIM_FRONT_1 )->IsEnabled() )
			{
				...
				CPoint	ptHotSpotClientMuzzleFlash = drawparmsTurret.SpriteViewToWindow( ptHotSpotMuzzleFlash, *pspriteviewVehicle );
...
CUnit * GetUnit (DWORD dwID)
{
	...
	return (theVehicleMap.GetVehicle (dwID));
}
...
CUnit * GetUnit (CSubHex const & sub)	// bldg takes priority
{
	...
	return (theVehicleHex.GetVehicle (sub));
}
