...
{
	for ( int i = 0; i < 256; ++i )
		g_afixSin[ i ] = sin( i * 2 * 3.14159265 / 256 );

	for ( i = 0; i < 256; ++i )
		g_afixCos[ i ] = cos( i * 2 * 3.14159265 / 256 );
}
	...
	CPoint const & ptWindow ) const
{
	...
	BYTE 	const *pbySrc 		= GetDIBPixels();

	if ( ! pbySrc )
		return FALSE;

	pbySrc += m_iBytesPerPixel * ( ptDib.y * Width() + ptDib.x );

	return memcmp( pbySrc, pbyMagenta, m_iBytesPerPixel );
}
	  ...
	  */
			...
			case 2 : {
				...
				while ( nCols-- )
				{
					...
					int iDist = iSrcWBytes * iV + 2 * iU;

					if ( 0 <= iDist && iDist < iLength )
						if ( * ((WORD *) (pSrc+iDist) ) != * ((WORD *) pbyMagenta) )
							* ((WORD *) pDst) = * ((WORD *) ( pSrc+iDist ) );
			...
			case 3 : {
				...
				while ( nCols-- )
				{
					...
					int iDist = iSrcWBytes * iV + 3 * iU;

					if ( 0 <= iDist && iDist < iLength )
						if ( memcmp( pSrc + iDist, pbyMagenta, 3 ))
							memcpy( pDst, pSrc + iDist, 3 );
			...
			case 4 : {
				...
				while ( nCols-- )
				{
					...
					int iDist = iSrcWBytes * iV + 4 * iU;

					if ( 0 <= iDist && iDist < iLength )
						if ( * ((DWORD *) (pSrc+iDist) ) != * ((DWORD *) pbyMagenta) )
							* ((DWORD *) pDst) = * ((DWORD *) ( pSrc+iDist ) );
	...
	CSpriteView const & spriteview ) const
{
	...
	Fixed	fixNewX =  fixX * fixCos + fixY * fixSin;
	Fixed	fixNewY = -fixX * fixSin + fixY * fixCos;

	ptWindow.x = fixNewX.Round();
	ptWindow.y = fixNewY.Round();
	...
	CSpriteView const & spriteview ) const
{
	...
	ptView.x = fixNewX.Round();
	ptView.y = fixNewY.Round();
	...
	const
{
	...
	for ( i = 0; i < 4; ++i )
	{
		...
		fixNewX =  fixX * fixCos + fixY * fixSin;
		fixNewY = -fixX * fixSin + fixY * fixCos;

		aptVertex[i].x = fixNewX.Round();
		aptVertex[i].y = fixNewY.Round();
	}
	...
	int			  iSuperviewIndex )
{
	...
	if ( !bCompressed )
	{
		...
		{
			int	iHeight = *( 1 + ( int * )pbySrc );

			ASSERT( pbySrc + 2 * sizeof( int ) <= pbySrcEnd );
			ASSERT( pbyDst + 2 * sizeof( int ) <= pbyDstEnd );

			pbySrc += 2 * sizeof( int );
			pbyDst += 2 * sizeof( int );

			int	nHeaderBytes = iHeight * sizeof( int );

			ASSERT( pbySrc + nHeaderBytes <= pbySrcEnd );
			ASSERT( pbyDst + nHeaderBytes <= pbyDstEnd );

			int	* piRow 				 = ( int * )pbyDst;
			...
			{
				piRow[ iRow ] = nCompressedBytes;

				for ( ;; )
				{
					int	iSrcBytes, iDstBytes;
					...
					*( long * )pbyDst = iDstBytes;
					...
					BYTE const * pbySrcBlockEnd = pbySrc + iSrcBytes;

					ASSERT( pbySrcBlockEnd <= pbySrcEnd );

					// Process the sequence of data bytes
